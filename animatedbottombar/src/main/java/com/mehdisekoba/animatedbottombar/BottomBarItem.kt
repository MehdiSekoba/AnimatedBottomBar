 ...
 *
 * @property icon
 * The icon displayed for this destination. If [selectedIcon] is `null`,
 * the same icon is used for both the selected and unselected states.
 *
 * @property label
 * A localized, human-readable label describing this destination.
 * The label is not rendered by default because the component follows
 * an icon-only design, but it is always exposed to accessibility
 * services as the content description.
 *
 * @property selectedIcon
 * An optional icon displayed when this destination is selected,
 * such as a filled version of an outlined icon.
 * If `null`, [icon] is reused.
 *
 * @property badge
 * An optional badge displayed over the icon, such as an unread
 * message count. See [Badge].
 *
 * @property enabled
 * Controls whether this destination can be selected.
 * Disabled items appear visually dimmed and do not trigger
 * selection events.
 */
@Immutable
